function createFullSet() {
    ...
    for (var i = 0; percentages.length > i; i++) {
        var num_questions_section = i18n.getLength("root.sections.section[" + i + "].questions.question");
        for (var j = 0; num_questions_section > j; j++) set.questions.push({
            s: i,
            n: j
        });
    }
...
function createNewSet(blockLetter) {
    ...
    "z" == blockLetter ? percentages = [ 14, 24, 25, 16, 10, 11 ] : "a" == blockLetter ? percentages = [ 100, 0, 0, 0, 0, 0 ] : "b" == blockLetter ? percentages = [ 0, 100, 0, 0, 0, 0 ] : "c" == blockLetter ? percentages = [ 0, 0, 100, 0, 0, 0 ] : "d" == blockLetter ? percentages = [ 0, 0, 0, 100, 0, 0 ] : "e" == blockLetter ? percentages = [ 0, 0, 0, 0, 100, 0 ] : "f" == blockLetter && (percentages = [ 0, 0, 0, 0, 0, 100 ]);
    set = {
        questions: []
    };
    var arrs = new Array();
    ...
    for (var i = 0; percentages.length > i; i++) {
        ...
        if (1 == Alloy.Globals.purchases[1] || 1 == Alloy.Globals.purchases[Number(i + 2)]) num_questions_section = i18n.getLength("root.sections.section[" + i + "].questions.question"); else {
        ...
        weight += Math.floor(section_weight);
        var arr = range(num_questions_section);
        while (weight > set.questions.length) {
    ...
    if (questions_left > 0) {
        ...
        for (var j = 0; questions_left > j; j++) {
...
function getQuestionNumberById(sec, num) {
    ...
    for (var i = 0; sec_length > i; i++) if (i18n.getString("root.sections.section[" + sec + "].questions.question[" + i + "]").id == num) {
...
function addQuestion(num) {
    ...
    var number;
    number = 1 == Alloy.Globals.purchases[1] || 1 == Alloy.Globals.purchases[section + 2] ? set.questions[num].n : set.questions[num].n;
...
function range(start, count) {
    ...
    var foo = [];
    for (var i = 0; count > i; i++) foo.push(start + i);
    return foo;
}
...
if (!Ti.App.Properties.hasProperty("seeded")) {
    ...
    for (var i = 0, j = titles.length; j > i; i++) {
...
Ti.App.addEventListener("newStudy", function() {
    ...
    2 == settingsJson.question_set ? createNewSet("a") : 3 == settingsJson.question_set ? createNewSet("b") : 4 == settingsJson.question_set ? createNewSet("c") : 5 == settingsJson.question_set ? createNewSet("d") : 6 == settingsJson.question_set ? createNewSet("e") : 7 == settingsJson.question_set ? createNewSet("f") : createNewSet("z");
});

Ti.App.addEventListener("newPractice", function() {
    Ti.API.debug("newPractice");
    type = "Practice Exam";
    num_questions = 100;
    time = 60 * 60 * Alloy.Models.settings.toJSON().timer;
    createNewSet("z");
});

Ti.App.addEventListener("newFull", function() {
    createFullSet();
});
